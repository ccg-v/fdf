...
/*   Updated: 2023/07/31 17:50:08 by ccarrace         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_menu(t_fdf *fdf)
{
	...
	if (fdf->map->projection == 0)
		mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, x, y += 50, 0xffffff, \
			"I     : isometric view");
	else if (fdf->map->projection == 1)
		mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, x, y += 50, 0xffffff, \
			"T     : top view");
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, x, y += 20, 0xffffff, \
		"arrows: shift map");
	mlx_string_put(fdf->mlx_ptr, fdf->win_ptr, x, y += 20, 0xffffff, \
		"+/-   : zoom in/out");
}
